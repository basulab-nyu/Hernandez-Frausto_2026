 function [sn,psdx,ff] = get_noise_fft2(Y,options)
        % [sn,psdx,ff] = get_noise_fft2(Y,options)
        %
        %
        % Edited 1/15/2019 by Jason J. Moore to be more memory efficient
        %
        ...
        if dims > 1
            ...
            if ~split_data
                for ind = 1:ceil(d/Nb) 
                    ...
                    psdx_temp = (1/(Fs*N)) * abs(xdft).^2;
                    psdx_temp(:,2:end-1) = 2*psdx_temp(:,2:end-1) + eps;
                    %SN{ind} = mean_psd(psdx(:,indf),method);
                    switch lower(method)
                        case 'mean'
                            SN{ind}=sqrt(mean(psdx_temp(:,indf)/2,2));
                        case 'median'
                            SN{ind}=sqrt(median(psdx_temp(:,indf)/2,2));
                        case 'logmexp'
                            SN{ind} = sqrt(exp(mean(log(psdx_temp(:,indf)/2),2)));
                        otherwise
